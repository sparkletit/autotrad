...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!rpcUrl || blockNumber === undefined) {
      return fail('缺少必要参数: rpcUrl 或 blockNumber', 400);
    }
    ...
    try {
      ...
      const healthCheck = await serverFetch(`${ANVIL_API_URL}/health`, { method: 'GET', signal: controller.signal as any }, 5000);
      
      clearTimeout(timeoutId);
      
      if (!healthCheck.ok) {
        return fail('Anvil API 服务未就绪，请稍候重试。', 503);
      }
    } catch (healthError: any) {
      if (healthError.name === 'AbortError' || healthError.code === 'ECONNREFUSED') {
        return fail('Anvil API 服务未就绪，请稍候重试。如果问题持续，请检查 anvil-api 容器是否正在安装 Foundry。', 503);
      }
      throw healthError;
    }
    ...
    try {
      response = await serverFetch(`${ANVIL_API_URL}/start`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ rpcUrl, blockNumber, chainId, loadState }),
        signal: (() => { const controller = new AbortController(); setTimeout(() => controller.abort(), 30000); return controller.signal; })(),
      }, 30000);
    ...
    if (!response.ok || !result.success) {
      return fail(result.error || '启动 Anvil Fork 失败', response.status || 500);
    }

    return ok({
      message: 'Anvil Fork网络已启动',
      config: { ...result.config, rpcUrl: ANVIL_RPC_URL },
    });
  } catch (error) {
