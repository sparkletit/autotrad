...
export async function GET(request: NextRequest) {
  try {
    ...
    try {
      const apiResponse = await serverFetch(`${ANVIL_API_URL}/status`, { method: 'GET', headers: { 'Content-Type': 'application/json' } }, 5000);
      ...
      if (apiResponse.ok) {
        ...
        if (apiResult.success && apiResult.isRunning) {
          ...
          return ok({ isRunning: false, message: 'Anvil Fork网络未运行' });
        }
      }
    } catch (err) {
    ...
    // 备选方案：直接连接 RPC 检查
    try {
      const response = await serverFetch(ANVIL_RPC_URL, {
        method: 'POST',
      ...
      if (response.ok) {
        ...
        if (data.result) {
          ...
          // 获取区块号
          try {
            const blockResponse = await serverFetch(ANVIL_RPC_URL, {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
          ...
          return ok({ isRunning: true, chainId, blockNumber, rpcUrl: ANVIL_RPC_URL, message: 'Anvil Fork网络运行中' });
        }
      }
    } catch (err) {
    ...
    return ok({ isRunning: false, message: 'Anvil Fork网络未运行', error: error instanceof Error ? error.message : '未知错误' });
  }
}
