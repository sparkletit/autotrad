...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // 参数验证
    if (!pairAddress) {
      return fail('缺少参数：pairAddress', 400);
    }

    // 验证地址格式
    if (!pairAddress.match(/^0x[a-fA-F0-9]{40}$/)) {
      return fail('无效的地址格式，应为 0x 开头的 40 个十六进制字符', 400);
    }
    ...
    try {
      ...
      if (codeErrorMsg.includes('empty reader set') || codeErrorMsg.includes('failed to get account')) {
        return fail(
          `地址 ${pairAddress} 在 ${network} 网络上不存在或无法访问。
...
- 使用已知的有效 Pair 合约地址`,
          400,
          { details: { pairAddress, network, rpcUrl, suggestion: '可以尝试在 BSC 或其他网络上查询该地址' } }
        );
      }
      throw codeError;
    }
    ...
    if (!code || code === '0x') {
      return fail(
        `地址 ${pairAddress} 在 ${network} 网络上没有合约代码。可能原因：
...
- 检查地址是否正确`,
        400,
        { details: { pairAddress, network, rpcUrl, hasCode: code && code !== '0x', codeLength: code?.length || 0 } }
      );
    }
...
2. RPC 地址是否正确`;
    ...
    return fail(
      ...
      httpStatus,
      {
        ...
        details: { pairAddress, network, rpcUrl: getRpcUrl(network) },
      },
      {
        headers: {
          'Cache-Control': 'no-store, no-cache, must-revalidate, max-age=0',
...
export async function GET(request: NextRequest) {
  try {
    ...
    return fail(`RPC 连接失败: ${errorMessage}`, 500, {
