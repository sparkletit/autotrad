-- 查询最近90天内活跃的 PancakeSwap 交易池的可疑转账
-- 筛选条件：
...
-- 获取最近90天内活跃的 PancakeSwap V2 交易池
...
-- 获取近90天内包含 Swap 事件的交易 hash（需要排除这些交易）
recent_swaps AS (
...
-- 获取近90天内包含 Burn 事件的交易 hash（需要排除这些交易）
recent_burns AS (
    SELECT DISTINCT evt_tx_hash
...
-- 获取近90天内包含 Mint 事件的交易 hash（需要排除这些交易）
recent_mints AS (
    SELECT DISTINCT evt_tx_hash
...
tx_enriched AS (
    ...
    INNER JOIN bnb.transactions tx 
        ...
        AND tx.block_time >= NOW() - INTERVAL '90' DAY  -- 限制交易表的扫描范围
